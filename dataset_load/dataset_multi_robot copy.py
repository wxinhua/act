    ...
    def __getitem__(self, index):
        ...
        with h5py.File(dataset_path, 'r') as root:
            ...
            for cam_name in self.robot_infor['camera_names']:
                ...
                # print(f"image: {image_dict[cam_name].shape}")
                ...
                # image_dict[cam_name] = np.zeros(shape)
        ...
        # image_data = all_cam_images.float()
        ...
        # shape = (3,3,480,640)
        # image_data = torch.zeros(shape).float()
        # shape = (3,480,640)
        # depth_data = torch.zeros(shape).float()
        # shape = (8)
        # qpos_data = torch.zeros(shape).float()
        # shape = (50, 8)
        # action_data = torch.zeros(shape).float()
        # shape = (50)
        # is_pad = torch.zeros(shape).bool()
...
# old dataset
class OldEpisodicDataset(torch.utils.data.Dataset):
    def __init__(self, dataset_path_list, robot_infor, norm_stats, episode_ids, episode_len, chunk_size, rank=None, use_data_aug=False, act_norm_class='norm2', use_raw_lang=False, use_depth_image=False, exp_type='franka_3rgb'):
        super(OldEpisodicDataset).__init__()
        self.episode_ids = episode_ids
        self.dataset_path_list = dataset_path_list
        self.norm_stats = norm_stats
        self.episode_len = episode_len
