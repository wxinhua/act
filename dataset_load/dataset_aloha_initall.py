...
class DatasetAloha(torch.utils.data.Dataset):
    def __init__(self, episode_ids, files, norm_stats, num_episodes, chunk_size, robot_infor,
        ...
        for trial_file in self.trial_files:
            print('trial_file:',trial_file)
            image_dict, control_dict, base_dict, is_sim, is_compress = self.read_h5files.execute(trial_file)
            self.infor_set.append((image_dict, control_dict, base_dict, is_sim, is_compress))

        print("TOTAL TRIALS", len(self.infor_set))
        self.infor_set = self.infor_set[:num_episodes]

        assert self.num_episodes == len(self.infor_set)  ## sanity check that all files are loaded, remove if needed

        print('TOTAL TRIALS = num_episodes = ', len(self.infor_set))
    ...
    def __getitem__(self, idx):
        ...
        (image_dict, control_dict, base_dict, is_sim, is_compress) = self.infor_set[trial_idx]
        # image_dict, control_dict, base_dict, is_sim, is_compress = self.read_h5files.execute(trial_file,
        ...
        for cam_name in self.robot_infor['camera_names']:
            all_cam_images.append(image_dict[self.robot_infor['camera_sensors'][0]][cam_name][start_ts]) # rgb
            all_cam_depths.append([image_dict[self.robot_infor['camera_sensors'][1]][cam_name][start_ts]])
...
def load_data(dataset_dir, robot_infor, batch_size_train, batch_size_val, chunk_size, cutoff=0):
    ...
    val_dataloader = DataLoader(val_dataset, batch_size=batch_size_val, shuffle=True, pin_memory=False, num_workers=1,
                                prefetch_factor=1)
